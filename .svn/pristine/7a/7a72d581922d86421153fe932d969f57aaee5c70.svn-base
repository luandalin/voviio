...
<?php }if(isset($delete)){?>
 <a href="#" onclick="deletereport()">
  ...
  <h3 class="h3_fontstyle"> 
	<?php echo  lang('m_rpt_errorsOfVersion')?>
 </h3>
  <ul class="tabs2">
...
var myurl = "<?php echo site_url()?>/report/errorlog/geterroralldata/";
</script>
<!-- report -->
<script type="text/javascript">
var options;
$(document).ready(function() {
	options = {
            ...
            tooltip: {
                formatter: function() {
                    return this.series.name +': '+ this.y +'<br>'+'<?php echo lang("v_man_au_version");?>'+':'+this.x;
                }
            },
            credits: {
                enabled: false
    ...
    {		
    	...
    	jQuery.getJSON(myurl, null, function(data) { 
    	    ...
    	    for(var j=0;j<obj.length;j++)
    	    {
		    	...
		    	categories.push(obj[j].version_name);
    	    }
...
{	
	...
	{
		var reportname="errorversion";
	    var reportcontroller="errorlog";
	    var data={ 
...
{ 
	...
	{
		window.parent.deletereport("errorversion"); 	 	 
	}
	return false;
