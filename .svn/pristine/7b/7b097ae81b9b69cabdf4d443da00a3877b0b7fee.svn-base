...
function Post($serverURL, $data) {
   ...
   function getregioninfo($latitude,$longitude,$id)
  	{		
			...
			$client = $this->utility->Post(
			'http://lbs.cobub.com/index.php?jsonservice/GetAddressByCord/',
			$arrVal);
			//$arr = $client->__soapCall('GetAddressByCord',array('parameters'=>$params));
			//echo $client;
			if(!isset($client)){
				$ret =array(
					'flag'=>-8,
    ...
    function haveregioninfobyip($ip,$id)
  	{		
			...
			$arrVal["content"] = json_encode($params);
			$client = $this->utility->Post(
			'http://lbs.cobub.com/index.php?jsonservice/GetAddressByIP/',
			$arrVal);
			if (!isset($client))
			{
				$ret =array(
